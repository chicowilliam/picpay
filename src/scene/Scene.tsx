...
import type { Texture, DirectionalLight } from 'three';
import type { MotionState } from '../App';
import { samplePose } from '../story/chapters';
import { backTexture, cardTexture, layerTexture, phoneTexture } from './textures';
...
function Face({ width, height, radius, map, z, lit = false }: { width: number; height: number; radius: number; map: Texture; z: number; lit?: boolean }) {
  const geometry = useMemo(() => roundedFace(width, height, radius), [width, height, radius]);
  useEffect(() => () => geometry.dispose(), [geometry]);
  return <mesh geometry={geometry} position-z={z}>{lit ? <meshPhysicalMaterial map={map} metalness={.26} roughness={.24} clearcoat={.65} clearcoatRoughness={.22} envMapIntensity={1.1} /> : <meshBasicMaterial map={map} toneMapped={false} />}</mesh>;
}

function Card() {
  const maps = useMemo(() => ({ front: cardTexture(), back: backTexture() }), []);
  useEffect(() => () => { maps.front.dispose(); maps.back.dispose(); }, [maps]);
  return <group>
    <RoundedBox args={[3.65, 2.3, .065]} radius={.115} smoothness={4} bevelSegments={3}><meshPhysicalMaterial color="#198e49" metalness={.55} roughness={.25} clearcoat={.45} /></RoundedBox>
    <Face width={3.63} height={2.28} radius={.11} map={maps.front} z={.035} lit />
    <group rotation-y={Math.PI}><Face width={3.63} height={2.28} radius={.11} map={maps.back} z={.035} lit /></group>
...
function Phone({ spread, compact, motion }: { spread: RefObject<number>; compact: boolean; motion: RefObject<MotionState> }) {
  ...
  useFrame(() => {
    ...
    for (const layer of [balance.current, activity.current]) if (layer) { layer.visible = recompose > .001; layer.scale.setScalar(Math.max(.001, recompose)); }
  });
  return <group>
    ...
    <Face width={1.77} height={3.8} radius={.18} z={.119} map={maps.phone} />
    <Transfer motion={motion} compact={compact} />
...
function World({ motion, onReady, onFailure }: Omit<Props, 'paused'>) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (changed || (!state.paused && state.progress < .95)) invalidate();
  ...
  useFrame((_, delta) => {
    const state = motion.current;
    const pose = samplePose(state.progress, mobile);
    ...
    if (card.current) {
      ...
      card.current.position.set(pose.x, y + float, pose.z);
    ...
    if (perspective.fov !== fov) perspective.fov = fov;
    perspective.updateProjectionMatrix(); camera.lookAt(target);
    ...
    if (import.meta.env.DEV) {
      const diagnostic = window as unknown as { __sceneInfo: unknown };
